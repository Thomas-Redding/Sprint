...
ParseNode* skimClass(const Token* tokens, uint64_t n) {
    if (n < 4) {
        return nullptr;
    }
...
uint64_t isValidFunctionName(const Token* tokens, uint64_t n) {
    assert(n != 0);
...
ParseNode* skimFunction(const Token* tokens, uint64_t n) {
...
ParseNode* getParseTree(const Token* tokens, uint64_t n) {
...
int main(int argc, const char * argv[]) {
    ...
    ParseNode* tree = getParseTree(&tokenizedList[0], tokenizedList.size());
